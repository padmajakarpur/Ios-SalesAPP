...
@synthesize tableViewHome,indicator,teamarr;
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    NSMutableDictionary *params=[[NSMutableDictionary alloc]init];
      ...
      NSString*    urlString = [[NSString alloc]initWithFormat:@"%@geteventdetails.php?",[prefs objectForKey:@"Link"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(screenRect.size.width*0.02,0,screenRect.size.width*0.96,screenRect.size.height*0.16)];
    ...
    Srnolbl.text =[NSString stringWithFormat:@"Event Name:%@",[dict objectForKey:@"Event_name"]];
    ...
    mobilelbl.text =[NSString stringWithFormat:@"Event date: %@",[dict objectForKey:@"event_date"]];
    ...
    datelbl.text =[NSString stringWithFormat:@"Event time: %@-%@",[dict objectForKey:@"event_ftime"],[dict objectForKey:@"event_ttime"]];
    ...
    UILabel*  avalibleslots = [[UILabel alloc] initWithFrame:CGRectMake(screenRect.size.width*0.02,screenRect.size.height*0.125,screenRect.size.width*0.80,screenRect.size.height*0.035)];
    avalibleslots.font=[UIFont systemFontOfSize:screenRect.size.width*0.04];
    avalibleslots.textAlignment = NSTextAlignmentLeft;
    avalibleslots.text =[NSString stringWithFormat:@"Avaible slots: %@",[dict objectForKey:@"event_avalibleslots"]];
    avalibleslots.backgroundColor = [UIColor clearColor];
    avalibleslots.textColor=[UIColor blackColor];
    avalibleslots.lineBreakMode = NSLineBreakByWordWrapping;
    avalibleslots.numberOfLines = 0;
    [headerView addSubview:avalibleslots];
...
{
        EvenrtDatailsViewController *mainvc=[[EvenrtDatailsViewController alloc] initWithNibName:@"EvenrtDatailsViewController" bundle:nil];
    mainvc.dict=[teamarr objectAtIndex:indexPath.row];
    if ([[[teamarr objectAtIndex:indexPath.row]objectForKey:@"event_avalibleslots"] isEqualToString:@"0"]) {
        UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"Xrbia" message:@"No slots are avaible."delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert show];
    }
    else
